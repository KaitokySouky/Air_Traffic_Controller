...
<body>
 <%@ page import="java.sql.*" %>
    ...
    PreparedStatement pstDepart = conn.prepareStatement("SELECT idaeroport FROM aeroport WHERE nom = ?");
    pstDepart.setString(1, departureAirport);
    ResultSet rsDepart = pstDepart.executeQuery();

    int idaeroportdepart = 0;
    if (rsDepart.next()) {
        idaeroportdepart = rsDepart.getInt("idaeroport");
    } 

    
    PreparedStatement pstArrive = conn.prepareStatement("SELECT idaeroport FROM aeroport WHERE nom = ?");
    pstArrive.setString(1, arrivalAirport);
    ResultSet rsArrive = pstArrive.executeQuery();

    int idaeroportarrive = 0;
    if (rsArrive.next()) {
        idaeroportarrive = rsArrive.getInt("idaeroport");
    } 
    ...
    PreparedStatement volStmt = conn.prepareStatement("INSERT INTO vol (départ, arrivée, altitude, avion, heuredépart, heureArrivée) VALUES (?, ?, ?, ?, ?, ?)");
    volStmt.setInt(1, idaeroportdepart);
    volStmt.setInt(2, idaeroportarrive);
    volStmt.setInt(3, Integer.parseInt(altitude));
    volStmt.setInt(4, Integer.parseInt(aircraft)); 
    volStmt.setInt(5, Integer.parseInt(departureTime)); 
    volStmt.setInt(6, Integer.parseInt(arrivalTime));  
    volStmt.executeUpdate();
    volStmt.close();
